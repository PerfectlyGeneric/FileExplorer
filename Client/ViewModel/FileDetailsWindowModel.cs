...
namespace AplicationUI.ViewModel
{
    internal class FileDetailsWindowModel : CommonHelpers.ViewModelBase
    {
        ...
        public FileDetailsWindowModel(AiModelHandler aiService,File file, IEnumerable<string> globalTags)
        {
            _originalFile = file;
            ...
            YourTags = ConvertToSeparateStrings(_originalFile);
            Notes = _originalFile.Notes;    
        ...
        private void ExecuteSave(object SaveGiven)
        {
            ...
            if(!string.IsNullOrWhiteSpace(YourTags))
            {
                var tags = YourTags.Split(new[] { ',', ' '}, StringSplitOptions.RemoveEmptyEntries)
                                   .Select(tag => tag.Trim())
                                   .ToList();
                _originalFile.Tags = tags;
            }
            else
            {
                _originalFile.Tags = new List<string>();
            }  
        ...
        private async void ProcessSelectedFileAsync()
        {
            ...
            try
            {
                ...
                var currentTags = _originalFile.Tags ?? new List<string>(); 
